...
「音声がまた途切れました」)はコサイン類似度0.68〜0.93で、既定閾値0.8を概ね超えて正しくコメント
追加側に倒れます。一方、「同じ話題だが言い回しが大きく異なる言い換え」(例:「AIが少し早口だった」→
「ChatGPT Liveの喋る速度が速すぎる気がする」)は0.33〜0.79程度に留まり、既定閾値では新規Issue
作成側に倒れます(無関係な文への類似度0.23〜0.36と比べれば方向性としては妥当ですが、確実に
拾えるわけではありません)。既定値0.8は「言い回しの近い再報告はまとめる」「無関係な内容を誤って
統合しない」を優先した保守的な設定であり、表現の異なる言い換えまで積極的に拾いたい場合は閾値を
下げてください(誤統合のリスクとのトレードオフです)。

...
送信テキストと対象リポジトリのOpen Issueのタイトル(**タイトルのみ**、本文は使いません。理由は後述、
最大300件)を、ローカルの多言語文埋め込みモデル(`Xenova/paraphrase-multilingual-MiniLM-L12-v2`、
`@huggingface/transformers`、外部AI API不使用)でベクトル化し、コサイン類似度を比較します。

- `FEED_SIMILARITY_THRESHOLD`(既定値`0.65`)以上の既存Issueが見つかった場合、その中で最も類似度が
...
**なぜタイトルのみで比較するか**: 当初は既存Issueの「タイトル+本文」を埋め込んでいましたが、
実運用で「落ち着いた感じで、会話には参加してほしい」(Issue #11)、「もっと落ち着いて話す
キャラクターがいい」(#14)、「落ち着いて話してくれると嬉しい」(#15)、「落ち着いた口調で話す
ほうが好みです」(#16)という、人間から見て明らかに同じ要望が**4件とも別Issueとして作成される**
不具合が発生しました。原因を調査したところ、`/feed`が作成するIssue本文は「## 自動判定」
「- 分類: …」「- 送信日時: …」といった定型文がほとんどを占め、実際のフィードバック本文は
ごく一部です。この定型文(全Issueでほぼ同一)が埋め込みベクトルを支配し、本来の類似度シグナルを
薄めてしまっていました。実測では、新規テキストと既存Issueの生テキスト同士の類似度は0.79だったのに
対し、実際の「タイトル+本文」同士では0.39まで低下していました。タイトルのみ(`/feed`が作成する
Issueのタイトルは「[分類] +フィードバック本文の先頭」で定型文を含まない)に切り替えたところ、
同じ組み合わせで類似度0.73〜0.84、無関係なIssueとの類似度は0.28程度と、明確に分離できることを
確認しました。分類プレフィックス(`[改善提案]`等)を含めたままにしているのは、それを外すと
無関係なIssueとの類似度も上がってしまい(0.49程度)、分離性が悪化することを確認したためです。

**実モデルでの検証結果(参考値、タイトル同士の比較)**: 「ほぼ同じ文言の再報告」は概ね0.68〜0.93、
「同じ話題だが言い回しが大きく異なる言い換え」は0.33〜0.84、無関係な内容は0.23〜0.49でした。

運用中に閾値を2度調整した経緯があります。

1. 当初の既定値0.65で、「大喜利が得意で居て欲しい」(Issue #18)と「お笑いに長けているとよい」
   (#19)のような、人間から見て明らかに同じ要望が類似度0.59と閾値のすぐ下で取りこぼされる
   事例が発生しました。取りこぼしを減らすため、リポジトリオーナーの判断で既定値を**0.49**へ
   引き下げました。
2. その直後、0.49の運用で**実際に誤統合が発生**しました。「ラグい」(動作遅延の技術的な不具合
   報告)という、Issue #18(「大喜利が得意で居て欲しい」というキャラクター性格の話)とは
   全く無関係な内容が、類似度**0.61**でIssue #18へコメント統合されてしまいました。
   「ラグい」のような短い単語は埋め込みが不安定になりやすく、閾値だけでは無関係な内容の
   混入を防ぎきれないことが実証されました。この結果を受け、リポジトリオーナーの判断により
   既定値を**0.65**へ戻しています(取りこぼしより誤統合の方が実害が大きいと判断)。

「大喜利/お笑い」(0.59)のような取りこぼしは既定値0.65でも再発しえますが、現時点では
無関係な内容を誤って統合しないことを優先しています。閾値の再調整を検討する場合は、この経緯
(0.61での実際の誤統合)を踏まえてください。
...
ユーザーごとに`FEED_COOLDOWN_SECONDS`(既定300秒=5分)に1回のクールダウンのみを設けています
(同一ユーザーからの同時多重実行も1件のみ処理されます)。実行権限自体はサーバー参加者全員に
開放されています。動作確認で`/feed`を連続実行したい場合は、`.env`で`FEED_COOLDOWN_SECONDS`を
一時的に短くしてください(公開運用に戻す際は必ず元に戻してください)。クールダウン状態は
プロセス内メモリのみで保持しているため、Botを再起動すればその場でリセットされます。
...
| 27 | 類似Issueへのコメント追加 | 24で作成したIssueと似た内容で`/feed content:<類似する文言>`を実行(`FEED_SIMILARITY_THRESHOLD`未設定なら既定0.65) | 新規Issueは作成されず、既存Issueへコメントが追加される。返信に`関連Issue: #N`が表示される |

...
**実モデルでの検証結果(参考値)**: 実際に`Xenova/paraphrase-multilingual-MiniLM-L12-v2`で日本語の
短文ペアを埋め込んで類似度を確認したところ、「ほぼ同じ文言の再報告」(例:「音声が途切れる」→
「音声がまた途切れました」)はコサイン類似度0.68〜0.93、「同じ話題だが言い回しが大きく異なる
言い換え」(例:「AIが少し早口だった」→「ChatGPT Liveの喋る速度が速すぎる気がする」)は
0.33〜0.79、無関係な内容は0.23〜0.36でした。当初の既定値0.8では、実運用で実際に
「落ち着いた感じで、会話には参加してほしい」(Issue #11)と「もっと落ち着いて話すキャラクターが
いい」(Issue #14, 類似度0.68)のような、人間から見て明らかに同じ要望の言い換えが別Issueとして
作成される事例が発生しました。無関係な内容(0.23〜0.36)とは十分に分離できる範囲で、
「拾いたい言い換え」をより多くカバーできるよう既定値を**0.65**へ引き下げています。
それでもなお表現差の大きい言い換えを取りこぼす場合や、逆に誤統合が増える場合は、環境変数で
調整してください。

...
- `FEED_SIMILARITY_THRESHOLD`(既定値`0.49`)以上の既存Issueが見つかった場合、その中で最も類似度が
...
当初は既定値0.65としていましたが、実運用で「大喜利が得意で居て欲しい」(Issue #18)と
「お笑いに長けているとよい」(#19)のような、人間から見て明らかに同じ要望が類似度0.59と
閾値のすぐ下で連続して取りこぼされる事例が発生しました。誤統合のリスクが上がることを承知の
うえで、リポジトリオーナーの判断により既定値を**0.49**へさらに引き下げています。これは「無関係な
内容」として観測した類似度の上限(0.49)とほぼ同じ水準であり、**言い回しが大きく異なるだけの
無関係な内容が誤って統合される可能性が以前より高くなっています**。実際に「キャピキャピしないで
ほしい」(#17)と「落ち着いた口調で話すほうが好みです」(#16、類似度0.4869)は、この閾値では
僅差で統合されないぎりぎりの水準です。誤統合が目立つ場合は閾値を引き上げてください。
